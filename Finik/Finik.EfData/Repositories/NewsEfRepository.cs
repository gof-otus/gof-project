...
public class NewsEfRepository : INewsDbRepository
{
    ...
    public async Task<IReadOnlyList<News>> GetAllNews() => await _dbContext.Set<News>().Include(n => n.Author).ToListAsync();

    public async Task<News?> GetNews(int id) => await _dbContext.Set<News>().Include(n => n.Author).SingleOrDefaultAsync(n => n.Id == id);
